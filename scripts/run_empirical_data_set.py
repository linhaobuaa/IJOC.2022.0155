...
try:
    ...
    pass
for i in range(0, 10):
    c = subprocess.run(["python", "main.py", "--trim_coefficient", "%.8f" % trim_coefficient, "--lr", "%.8f" % lr, \
        "--num_epochs", "%d" % num_epochs, "--gmm_k", "%d" % gmm_k, "--latent_dim", "%d" % latent_dim, "--latent_categories", \
        "%d" % latent_categories, "--lambda_seq_proximity", "%.8f" % lambda_seq_proximity, "--lambda_energy", "%.8f" % lambda_energy, \
        "--lambda_cov_diag", "%.8f" % lambda_cov_diag, "--lambda_disc", "%.8f" % lambda_disc, "--use_different_lr", str(use_different_lr), \
        "--use_cuda", str(use_cuda), "--input_data_path", input_data_path, "--index_id_label_path", index_id_label_path, \
